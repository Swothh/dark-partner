...
export const Event: Events = {
    ...
    run: async (client, interaction: Interaction) => {
        try {
            if (!interaction.isButton() || !interaction.customId.startsWith('botlist.add_bot_button')) return;
            ...
            const modal = await create_modal.build(`botlist.add_bot_interaction-${Math.floor(Math.random() * 1000)}`, {
                modal: new ModalBuilder()
                    ...
                    .addComponents(
                        new ActionRowBuilder<TextInputBuilder>()
                            .addComponents(
                                new TextInputBuilder()
                                    .setCustomId('botlist.add_bot_interaction.input')
                        ...
                        new ActionRowBuilder<TextInputBuilder>()
                            .addComponents(
                                new TextInputBuilder()
                                    .setCustomId('botlist.add_bot_interaction.input_2')
            ...
            const botId = modal.fields.getTextInputValue('botlist.add_bot_interaction.input');
            const prefix = modal.fields.getTextInputValue('botlist.add_bot_interaction.input_2');
            ...
            if (interaction.guild.members.cache.get(botId)) return await modal_interaction.error(locale('events.botlist.addBot.errors.bot_in_server'));
